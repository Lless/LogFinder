...
class TextFinder {
    static List<Long> find(Reader textReader, String pattern) {
        final int smBufLen = pattern.length()-1;
        char[] buf = new char[max(pattern.length()*5,1024)]; //keeps loaded chars
        char[] smBuf = new char[smBufLen];//keeps last part of previous buffer
